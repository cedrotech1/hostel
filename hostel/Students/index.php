...
$regnumber=$_SESSION['regnumber'];
  if ($regnumber) {
    ...
    $sql = "SELECT email FROM info WHERE regnumber='$regnumber'";
    $result = mysqli_query($connection, $sql);
    $row = mysqli_fetch_assoc($result);
    if ($row) {

      $sql1 = "SELECT email FROM info WHERE regnumber='$regnumber' and picture!='' and status!='rejected'";
    ...
    '>
        <div style='
            ...
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.2);
        '>
            <h2>Your You are not authonticated </h2>
            <p>you are not authonticated to see this page, you must pass authontication page first</p>
            <button onclick='redirectToHome()' style='
                padding: 10px 20px;
                font-size: 16px;
                background-color: #4CAF50;
...
if (isset($_POST['image']) && isset($_POST['regnumber'])) {
    ...
    if (file_put_contents($target, $image)) {
        ...
        $sql = "UPDATE info SET picture='$target',updatedat='$currentDateTime' WHERE regnumber='$regnumber'";
